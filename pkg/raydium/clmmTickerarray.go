...
type TickArray struct {
	...
	InitializedTickCount uint8            // 1 byte，不需要 bin tag
	_                    [115]byte        `bin:"skip"` // padding
}

type TickState struct {
	Tick                    int32              `bin:"le"`   // 4字节
	LiquidityNet            int64              `bin:"le"`   // 8字节
	_                       [8]byte            `bin:"skip"` // 跳过高8字节
	LiquidityGross          uint128.Uint128    `bin:"le"`   // 16字节
	FeeGrowthOutsideX64A    uint128.Uint128    `bin:"le"`   // 16字节
	FeeGrowthOutsideX64B    uint128.Uint128    `bin:"le"`   // 16字节
	RewardGrowthsOutsideX64 [3]uint128.Uint128 `bin:"le"`   // 48字节
...
func (p *CLMMPool) GetTickArrayAddresses() ([]solana.PublicKey, error) {
	startIndexArray := p.getInitializedTickArrayInRange(10) // 与 Raydium 保持一致，获取 7 个 tick array
...
func (p *CLMMPool) ParseExBitmapInfo(data []byte) {
	...
	// 解析 positiveTickArrayBitmap
	// EXTENSION_TICKARRAY_BITMAP_SIZE 个数组，每个数组包含 8 个 uint64
	...
	for i := 0; i < EXTENSION_TICKARRAY_BITMAP_SIZE; i++ {
		...
		data = data[64:] // 跳过 8 个 uint64 (64字节)
	}
	bitmap.PositiveTickArrayBitmap = positiveBitmaps

	// 解析 negativeTickArrayBitmap
...
// checkTickArrayIsInitialized 检查 tick array 是否已初始化
...
// getTickArrayBitIndex 获取 tick array 在 bitmap 中的位置
...
// getTickCount 获取每个 tick array 包含的 tick 数量
...
func SearchLowBitFromStart(
	...
	// Multiply each result by tickCount
...
func SearchHighBitFromStart(tickArrayBitmap [16]uint64,
	...
	// Reverse and add negativeTickArrayBitmap
	for i := len(exTickArrayBitmap.NegativeTickArrayBitmap) - 1; i >= 0; i-- {
		tickArrayBitmaps = append(tickArrayBitmaps, MergeTickArrayBitmap(exTickArrayBitmap.NegativeTickArrayBitmap[i]))
	}

	// Add main bitmap slices
...
// GetBitmapTickBoundary 获取位图的tick边界
...
// ExtensionTickBoundary 获取扩展tick边界
