...
func (r *CPMMPool) BuildSwapInstructions(
	ctx context.Context,
...
func (pool *CPMMPool) GetPrice(amountIn math.Int, mintIn string) (math.Int, error) {
	...
	input := "base"
	if mintIn == pool.Token1Mint.String() {
		input = "quote"
	}
	...
	if !amountIn.IsZero() {
		// Calculate fee
		feeRaw = amountIn.Mul(LIQUIDITY_FEES_NUMERATOR).Quo(LIQUIDITY_FEES_DENOMINATOR)

		// Calculate amountInWithFee
		amountInWithFee := amountIn.Sub(feeRaw)
